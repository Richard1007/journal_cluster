...
def remove_anomaly(df):
    ...
    # IP_block = 0    
    #df = df[(df['DistributionChannel']=='anonymous') & ((df['IP_block']==0) | (pd.isna(df['IP_block'])))]
    print("Ignoring IP block and distribution channel")
    # keep responses with >=8 non-repetitive answers
    print(f"Original data has {df.shape[0]} rows.")
...
def process_text(df,nlp):
    ...
    def preprocess(text):
        ...
        for word,tag in tags:
            ...
            is_proper_noun = (tag == "NNP")
            if lemma not in english_stopwords and len(lemma)>2 and (not is_proper_noun):
                lemmas.append(lemma)
        return lemmas
    ...
    # we remove these words because they don't add any meaning (e.g., stopwords)
    # or because they are undetected antonyms.
    blacklist = ['well','somewhat','look','something','thing','sort','know','liker','maybe','none','everyone','dont','yes','idk','always','fairly','everybody','nothing','someone','quite','slightly','okay','sorry','belive','absolutely','expect','happen','seem','alright','simply','really','alot','believe','seriously','totally','kinda','possible','one','name','really','person','reply','untrendy','unhip','nonhipster','adult','human','bos']
    pronouns = ['john', 'richy', 'ralph', 'joshua', 'tommy', 'karen', 'jean', 'joe', 'linda', 'victor', 'chad', 'anne', 'william', 'kim', 'carrie', 'rick', 'ritch', 'butch']
    blacklist += pronouns
    ...
    (uq, cnts) = np.unique(df[P1_lemma+P2_lemma].values.sum(), return_counts=True)
    idx = np.where(cnts < 5)[0]
    print("Current cutoff is set to remove any count < 5")
    infrequent_words = set(uq[idx])
    def remove_infrequent(row):
        words = []
        for i, word in enumerate(row):
            if(not word in infrequent_words):
                words.append(word)
        return words
    ...
    # save all the word lemmas in one column and delete all temporary data
    sum_lists =  lambda r: list(itertools.chain.from_iterable(r.values))
    df["lemmas"] = df[P1_lemma+P2_lemma].apply(sum_lists, axis=1)
    
    #df = df.drop(P1+P2+P1_lemma+P2_lemma,axis=1)    
    df = df.drop(P1+P2,axis=1)    
    #print("TODO: drop P1, P2 here again later!")

    df = df[df.lemmas.apply(len) >0]
    print(f"After deletion of rows with too few (<=0) non-recognized answers: {df.shape[0]} rows")    
...
def construct_participant_level_df(df):
    ...
    df[P3_adjust]         = df[P1_emb+P2_emb+P3].apply(adjust_association,axis=1)  # adjust association rating P3 to nan for invalid responses
...
def get_membership_dist_from_C(df, df_brand, membership_col, metric):
    ...
    '''
    # mean concept membership is defined per the control
    brand_control = df_brand[df_brand.treatment =='no'][["brand"]+membership_col].set_index("brand")
    ...
    def calc_mem_dist(row):
        ...
        C_mean  = brand_control.loc[row.brand]
        par_vec = row[membership_col]
        return metric(par_vec, C_mean)
...
## Word Clustering
############################
def cluster_words_OLD(word_list,k,nlp):
...
def find_central_word(df_word,df_cluster):
    ...
    # ---- start fix
    #res = pd.DataFrame(df_cluster.label).join(res,how="left")
    #for idx in np.where(pd.isna(res.word))[0]:
    #    res.at[idx,'word'] = []
    # --- end fix
...
def calc_word_membership(df_word,df_cluster):
    '''Calculate distance of words against each centroid.
    ...
    # version 2.0
    M_centroids  = np.c_[[ x for x in df_cluster['centroid'].values]]
    M_word_embed = np.c_[[ x for x in df_word.embedding.values]]

    similarities = (1-scipy.spatial.distance.cdist(M_word_embed,M_centroids, metric='cosine'))
    
    
    colnames = [f'dist_from_centroid_{i}' for i in range(df_cluster.shape[0])]
    df_word[colnames] = similarities    
    return df_word
...
def calc_part_membership(df,df_word):
    ...
    # compute their average distance from the cluster centers
    words = df.lemmas
    word_memberships = np.array([df_word.loc[wordsi][membership_col].mean(axis=0) for wordsi in words])

    df[membership_col] = word_memberships
    return df

def calc_mem_var(df,df_word,membership_col, dist_fun='cosine'):
    
    '''
        Calculate the membership variation for each participant
        dist_fun must be a string to feed to scipy.spatial.distance.cdist
        https://docs.scipy.org/doc/scipy-0.17.0/reference/generated/scipy.spatial.distance.html
    '''
    def func(row):
        # OLD
        ...
        #    distances.append(dist_fun(w_mem,avg_mem))
        #
        # averaging the distances
        #return sum(distances)/len(distances)
        # v2.0
        avg_mem      = row[membership_col].to_numpy().astype(float)  # 100x1 participant's average
        M_word_embed = df_word.loc[row.lemmas][membership_col].values
        distances    = scipy.spatial.distance.cdist(M_word_embed, [avg_mem],dist_fun)
        return distances.mean()
    return df.apply(func,axis=1)

def count_words_in_cluster(df,df_word,k):
    '''Count the number of words in the response that belong to each cluster'''
    def func(row):
        res = np.zeros(k)
        
        for w in row.lemmas:
            label    = label_mapper[w]
            res[label] += 1
        return res
        
    label_mapper = df_word['label']
    return df.apply(func,axis=1)    
...
def vis_2d_brand_membership_C(df_brand,df_cluster,dim_list,dpi=60):
    '''visualize brand control group only in a set of quadrant plots'''
    nrow = (len(dim_list)-1)//3+1
    ncol = 3
    fig,axes = plt.subplots(nrow,ncol, figsize=(ncol*5,nrow*4),dpi=dpi,facecolor='white')
    axes = axes.flatten()
    xy_lims = []
...
def set_up_quadrant_spine(dim1,dim2,ax,x_loc=None,y_loc=None):
    ...
    # set text
    ax.text(xmax, y_loc, "MORE\n"+ dim1.upper(),horizontalalignment='left',verticalalignment='center',weight = 'bold',bbox=dict(facecolor='white', alpha=1))
    ax.text(x_loc, ymax, "MORE\n"+dim2.upper(),horizontalalignment='center',verticalalignment='bottom',weight = 'bold',bbox=dict(facecolor='white', alpha=1))
    ax.text(xmin, y_loc,"LESS\n "+dim1.upper(),horizontalalignment='right',verticalalignment='center',weight = 'bold',bbox=dict(facecolor='white', alpha=1))
    ax.text(x_loc, ymin,"LESS\n"+dim2.upper(),horizontalalignment='center',verticalalignment='top',weight = 'bold',bbox=dict(facecolor='white', alpha=1))
...
def get_membership_col(df_cluster):
     return ['dist_from_centroid_'+str(i) for i in range(df_cluster.shape[0])]